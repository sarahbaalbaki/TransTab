...
class MixtureOfExperts(nn.Module):
	'''MOE should be integrated with original transtab'''
	...
	def forward(self, x):
		...
		output = torch.einsum('bij,ebj->bij', gates.unsqueeze(2), expert_outputs)
		return output.sum(0)
...
class LabelSmoothing(nn.Module):
	...
	def forward(self, input):
		""" Apply label smoothing. Assumes that `input` is in class probabilities. """
		input_size = input.size(self.dim)  # number of classes
		smoothing_value = self.smoothing / (input_size - 1)
		one_hot = torch.full_like(input, smoothing_value)
		target = torch.scatter(
			one_hot, self.dim, input.argmax(dim=self.dim, keepdim=True), 1.0 - self.smoothing
		)
		return target
